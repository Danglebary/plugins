...
Distinct from the **content channel**: instruction-shaped text in the material an agent reviews — comments, docstrings, fixture text — carries no authority over it. Two delivery paths, both covered: hunks inlined into a reviewer's brief, and working-tree content the agent reads itself (the ad-hoc arm ships no diff artifact at all). Every agent refuses to obey it; whether an agent also *reports* it is scoped to convergence context (ADR 0008): a `/refactor` reviewer reports only as material owner — prompt artifacts to `prompt-expert`, everything else to `security-engineer` — so a single planted line cannot inflate the ranking, while the close-out pair, whose two returns are never ranked against each other, report a planted instruction they find. That rule never demotes repo authority arriving via the brief or read from the base tree; which authority a diff can rewrite remains ADR 0005's line-granular test. Its home is the `CONTENT-CHANNEL-PRINCIPLE.md` principle doc.
