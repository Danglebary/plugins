...
- [ ] The script scaffolds `.pirr/` and its deny-by-default `.gitignore` when absent, and writes the diff to `.pirr/diff.patch`.
...
The script must be reachable from skill prose via the plugin-root path convention (`${CLAUDE_PLUGIN_ROOT}`). The artifact path `.pirr/diff.patch` is a published contract — the deviation-fact-checker agent body names it; do not move it.

## Deviations

- Added a fourth "convention docs" kind to the `_shared/` reference-doc taxonomy (`plugins/pirr/CLAUDE.md`, `skills/_shared/README.md`) to index the new contract doc — the planned work created the doc, but classifying it required a category the taxonomy didn't have, and future shared procedures will follow it.
- (refactor) Added exit code 7 to the script's contract: it now refuses to write through a symlinked `.pirr/`, `diff.patch`, or `.gitignore`. Closes an arbitrary-file-write hole — a hostile repo could commit the artifact path as a symlink outside the tree, and the committed symlink passed the tracked-only dirty preflight. Documented in the DIFF-MATERIALIZATION.md exit-code table.
