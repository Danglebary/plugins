...
- Extended `/done`'s fact-checker brief with the planning-artifact hunk label — [DIFF-MATERIALIZATION.md](../../../../plugins/pirr/skills/_shared/DIFF-MATERIALIZATION.md) mandates it for every brief that hands over a files-store diff, so consuming the convention without it would breach the shared contract; the criteria list didn't name the brief.
