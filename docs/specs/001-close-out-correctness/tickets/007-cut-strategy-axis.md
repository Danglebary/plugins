...
- (refactor) The template's `merge`-key comment now points at the seam instead of enumerating readers — CLOSE-OUT.md's gated merge, whose Consumers section owns the list — because the enumeration had re-rotted at the moment of correction (omitting `/improve-codebase-architecture`'s gate-3 merge, the commonest executor of the ticket merge on the defer path). Supersedes criterion 3's literal "names both `/done` and `/retro`"; dogfood copy aligned.
- (refactor) The config seam now states its read contract in STORE.md — skills read `settings.toml` as prose, consult only the keys they name, and treat stale keys/comments as inert, never live instruction — with setup's refresh tolerance hoisted to the store-neutral "First run vs re-run" section citing it (was asserted locally in the files re-run step) and `/next-ticket`'s cut point citing it to disarm stale `strategy` prose in preserved configs.
