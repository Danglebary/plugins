...
- [ ] `grep -ri prd plugins/pirr/` matches only the migration note and any deliberate legacy-branch handling prose — nowhere else.
...
- Added a legacy-store routing sentence to `/setup`'s re-run detection and STORE.md's not-set-up check: a repo with `docs/prds/` and no `docs/specs/` predates the rename and routes to the migration note instead of being scaffolded a parallel store — new behavior no AC named, required so the rename doesn't turn legacy repos into false "first runs".
