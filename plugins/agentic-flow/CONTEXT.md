...
**Config**:
The per-repo configuration for `agentic-flow`. Files store: `docs/agentic-flow.toml` (also the store's structural marker); notion store: a config block in the `Agentic-Flow` root page body. Created by `/setup-agentic-flow` with all options present (defaults stated, alternatives noted). Currently holds branching strategy (`serial` vs `stacked`); future options append here. Skills update it as configuration choices materialize.
