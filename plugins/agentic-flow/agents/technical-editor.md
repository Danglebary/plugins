...
description: Reviews a code diff for cross-document coherence in a prose corpus — contract drift across must-stay-in-sync copies, terminology drift against the canonical vocabulary, reference rot, format-doc conformance, and prose whose claims the repo contradicts. Dispatched by /improve-codebase-architecture per the Reviewers manifest when the repo's product or spec surface is a substantial prose corpus.
...
If no candidates surface, output `_No editorial candidates._` and stop.
...
- **Don't reconcile by rewriting.** Cite the disagreeing copies and sketch which should win; the grilling loop in `/improve-codebase-architecture` does the design.
