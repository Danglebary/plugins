...
2. **Offer one commit** on the branch the close works on, naming the enumerated paths. Stage them explicitly — `git add <path> <path> …`, never `-A`, never `git add .` (blanket staging sweeps unrelated tree state into the close-out commit).
...
- **Store-artifact dirt, flip in the working tree (uncommitted)** — the closing skill's own interrupted close. That skill resumes: the flip's presence means every earlier store edit landed (the flip is last) — resume at the gated commit. If instead the flip is absent from tree and history, the crash was mid-close: resume from the first absent store-writing step, applying idempotently.
...
- **Implementation-path dirt, any** — not a close-out state. Refuse: implementation is committed before close-out runs.

## Consumers

`/done` (ticket close: ticket branch → PRD branch, steps 10–11) and `/improve-codebase-architecture` (refactor-pass close-out: same branch pair; its post-merge and general ad-hoc arms have no branch to merge and end at the commit gate). Each consumer states its bindings and cites this document for the mechanics.
