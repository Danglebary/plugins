...
**Cut point:** cut the ticket branch from the PRD branch — the same base the ticket's eventual diff resolves against (the ticket-scope row of [DIFF-MATERIALIZATION.md](../../_shared/DIFF-MATERIALIZATION.md)), so the cut point and the diff base cannot disagree.
