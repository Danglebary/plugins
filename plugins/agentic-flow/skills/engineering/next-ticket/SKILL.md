...
- **PRD state required**: `Open` (the active PRD per the store's active pointer), with its bootstrap landed — the branch `prd-<NNN>-<slug>` exists (local or remote) plus its follow-through: files, the planning commit; notion, the `Branch`/`Diff base` properties (the landed test — inline copy of STORE.md's branch-link state tests, cited per ADR-0002)
...
**PRD-branch precondition (before anything is cut):** the PRD's bootstrap must have landed — the branch exists (local or remote) *and* its follow-through did too: files, the planning commit is on the branch; notion, the `Branch`/`Diff base` properties are written (the landed test — inline copy of STORE.md's branch-link state tests, cited per ADR-0002). If the branch is missing or half-landed, **refuse the flip and the cut** and name the fix: re-invoking `/to-tickets` on this PRD re-offers its re-enterable bootstrap — only the missing pieces, nothing else. This skill never creates the PRD branch: a locally-minted branch would skip the bootstrap's serialize-ticketing preconditions and, on the files store, leave the planning artifacts without their first commit.

**Cut point:**
- Read the config's `branching.strategy` value.
- If `serial`: cut the ticket branch from the PRD branch — the same base the ticket's eventual diff resolves against (the ticket-scope row of [DIFF-MATERIALIZATION.md](../../_shared/DIFF-MATERIALIZATION.md)), so the cut point and the diff base cannot disagree.
- If `stacked`: cut the ticket branch from the previous ticket's branch (typically the most recently `Done` ticket). The first ticket of a PRD has no previous ticket — under both strategies it cuts from the PRD branch.
...
**Dependency-reachability check (before cutting, both strategies):** for every dependency of the new ticket, verify its work is reachable from the intended cut point — find the dependency's close commit (or its branch tip if the branch still exists) and check ancestry with `git merge-base --is-ancestor`. This check stays this skill's own single-command prose; DIFF-MATERIALIZATION.md's script is single-purpose and not invoked here. If a `Done` dependency's work isn't reachable, its close-out didn't finish — **stop and route through [CLOSE-OUT.md](../../_shared/CLOSE-OUT.md)'s recovery**, binding the dependency's branch pair (its ticket branch → the PRD branch): the resting-state table discriminates, so an uncommitted `→ Done` flip resumes the dependency's gated store commit first, and only then comes the gated merge per the configured convention. Never a merge-only shortcut — merging a branch whose close-out edits are uncommitted lands a tree that contradicts its own status. Once the recovery lands, re-run the reachability check, then cut. Cutting from a stale parent silently builds the new ticket on a tree missing its dependency's work — this happened and shipped a real defect.
...
- **Don't create the PRD branch.** A missing `prd-<NNN>-<slug>` means the bootstrap never landed — refuse and point at `/to-tickets`' re-enterable bootstrap. Minting the branch here skips the serialize-ticketing preconditions and the bootstrap's follow-through (the planning commit / the notion property writes).
