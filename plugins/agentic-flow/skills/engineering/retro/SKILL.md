...
11. **Clear the active pointer** *if it names this PRD* (files: delete `docs/prds/.active` — its one line is the PRD directory name, `<NNN>-<slug>`, the form to compare against; notion: uncheck the row's `Active`). If the user has manually pointed it at a different PRD, leave it alone. This edit comes *before* the flip: the status flip is the close's **last** store edit — the published invariant of [CLOSE-OUT.md](../../_shared/CLOSE-OUT.md) that step 3's recovery routing reads.
