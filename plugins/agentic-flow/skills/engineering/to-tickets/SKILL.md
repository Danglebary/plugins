...
8. **Record the branch link.** The PRD branch is `prd-<NNN>-<slug>`. Files: implicit — the PRD directory name already encodes it; nothing to write. Notion: write `Branch = prd-<NNN>-<slug>` and `Diff base` on the PRD row — this is what `/done` and `/retro` read to find the git diff range; with directories gone, the link must be explicit. Resolve `Diff base` to the repo's **actual** default branch via the shared default-branch procedure in [DIFF-MATERIALIZATION.md](../../_shared/DIFF-MATERIALIZATION.md) — don't hard-code `main`, and never fall back to the current branch (`/retro` reads the property literally, so a `Diff base` resolved from the wrong branch silently mis-scopes the PRD diff; a `master`/`trunk` repo must resolve to its real default, not a guess).
