...
**The bootstrap has landed** when the linked branch — `spec-<NNN>-<slug>`, or its legacy `prd-` twin per STORE.md's branch-link fallback — exists (local or remote) *and* its follow-through did too — the planning commit is on the branch. (The landed test — inline copy of STORE.md's branch-link state tests, cited per ADR-0002.) The discriminator checks the follow-through, not just the cut — a crash between them must route to re-entry, not refusal.
