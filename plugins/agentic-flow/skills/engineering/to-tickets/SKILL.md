...
**The bootstrap has landed** when the branch `prd-<NNN>-<slug>` exists (local or remote) *and* its follow-through did too — files: the planning commit is on the branch; notion: the `Branch`/`Diff base` properties are written. (The landed test — inline copy of STORE.md's branch-link state tests, cited per ADR-0002.) The discriminator checks the follow-through, not just the cut — a crash between them must route to re-entry, not refusal.
...
1. **Identify the PRD.** If the user hasn't specified, ask. Default to the most recent `Drafting` PRD if there's an obvious one. If the PRD is `Open` with tickets, check whether the bootstrap has landed (see State contract): any missing piece — no branch, or a branch without its planning commit (files) / `Branch`+`Diff base` writes (notion) — routes to **Bootstrap re-entry** (below); fully landed refuses (frozen scope). Otherwise refuse if the PRD's status isn't `Drafting`. For a `Drafting` PRD, also re-verify its number while renumbering is still cheap: the `<NNN>` must be unique across the planning directories and `prd-<NNN>-*` branch names (the branch-aware scan of `/to-prd` step 1, observed live here — an offline-minted number can collide with a PRD in flight elsewhere, and a locked number is immutable). On collision, renumber the `Drafting` PRD before proceeding.
