...
A dispatching skill's check, before any lens is dispatched, that every named `subagent_type` resolves against the available agent types — refusing and naming the unresolved ones rather than proceeding. The deterministic half of the coverage claim: an unknown type hard-errors at the tool boundary, so absence is observable *before* the work, never after it. `/refactor` has carried one since its Reviewers-manifest step; `/done` gained one at its close-out pair, the site of the recorded degradation (ADR 0006). Because it refuses before *any* lens is dispatched, a run it stops has no partial coverage to report — which is why a persisted **Dispatch record** never pairs a returned lens with an unresolved one.
_Avoid_: "dispatch validation" — the check is name resolution only, never output inspection

**Dispatch record**:
The dispatching skill's positive account of which lenses ran, kept by `/refactor` and `/done`, composed from the intended lens list *before* the **Resolution preflight** runs, and **persisted** in the ticket's running-retro entry rather than left in chat. Four result states: **returned** (a return carrying its **Partial verdict** register), **degraded** (a return without one — never counted as returned), **refused** (declined or errored after dispatch), **unresolved** (failed the preflight; nothing was dispatched). Emission is mandatory, detail gap-only: the clean case is one line, and only the three non-clean states carry detail. Where no retro entry exists to receive it — a preflight refusal, or a `/refactor` pass scoped to no ticket — the emission is the record. An **attestation**, not a verification — a successful dispatch's return carries no agent identity, and the subagent cannot self-identify either, so nothing downstream can audit it (fired 2026-07-19). Its weight comes from the **Resolution preflight** and the anti-substitution rule beside it, not from its own authority (ADR 0006).
