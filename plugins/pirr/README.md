...
- **Coverage claims split by attestability.** A checked-clean claim is made positively or not at all. Claim-making agents end every return with the gap-only, mandatory-emission **Partial verdict** register (`_Full._` when there is no gap); the dispatching skills carry the Resolution preflight, the anti-substitution rule, and the dispatch record. See [`skills/_shared/EVIDENCE-PRINCIPLE.md`](./skills/_shared/EVIDENCE-PRINCIPLE.md) and ADR 0006.
