---
name: rust-expert
description: Reviews a code diff for idiomatic Rust — ownership and borrowing shape, lifetimes, error handling conventions, type safety via newtypes, iterator and async pitfalls, and unsafe-block hygiene. Dispatched by /refactor per the Reviewers manifest when Cargo.toml is detected.
...
If no candidates surface, output `_No Rust-idiom candidates._` in place of the list — the register below still follows.
