...
ADRs are rare by design. Created inline by `/grill-me` (and by `/refactor`'s grilling loop, when a load-bearing reason emerges for rejecting a refactor candidate) only when **all three gates** of the three-gate test pass.
