...
- [DIFF-MATERIALIZATION.md](./DIFF-MATERIALIZATION.md) — the diff-materialization contract: the invoking skill resolves `<base>`/`<head>` per scope; the plugin-shipped script (`scripts/materialize-diff.sh`) owns the git mechanics, preflights, and the `.pirr/diff.patch` artifact
