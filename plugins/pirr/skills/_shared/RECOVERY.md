...
2. Re-run the script. Never reuse a leftover `diff.patch` (staleness is unverifiable) and never hand-roll the diff. **The stash is path-limited and carries no `-u`, so the crashed run's *untracked* store artifacts — a minted ADR, a first `retro.md` — stay in the tree and the re-run refuses with exit 8** ([DIFF-MATERIALIZATION.md](./DIFF-MATERIALIZATION.md)). That is correct, not a fault: they are legitimately part of the close. Classify them per the resuming skill's exit-8 arms and re-invoke with `--allow-untracked` naming them. Widening the stash to `-u` instead would tax every routine close-out with a round trip through this recipe's crash window, for files that cannot affect a `base...head` diff.
...
- **Flip absent; the running retro has uncommitted content** (a modified `retro.md` still in running form) — **refuse: the running retro must be committed before the synthesis consumes it** (the committed-running-retro precondition `/retro` step 3's preflight owns; synthesizing over uncommitted entries destroys them with no recovery path). This dirt belongs to an earlier ticket close **or refactor pass** — the running retro has two writers — and the ticket's `done` flip discriminates: an **uncommitted** ticket `done` flip alongside it is an interrupted `/done` close-out, so point at re-running `/done`; a **committed** flip with the retro still dirty is an interrupted `/refactor` pass, which writes its dispatch record at step 9 *before* its gates, so point at re-running `/refactor` (its gate 2 commits the record). Neither shape applies → have the user commit the running-retro edits, then re-run `/retro`.
