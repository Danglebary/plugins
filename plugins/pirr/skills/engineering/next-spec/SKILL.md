...
Exploration skill at the front of the pirr workflow. Output is a *conversation*, not a document. The next skill in the chain is a **high-level `/grill-me`** that stress-tests the chosen direction; only after that does `/to-spec` capture it as a draft spec. The full planning chain: `/next-spec` → `/grill-me` (high-level) → `/to-spec` → `/grill-me` (detail) → `/to-tickets`.
