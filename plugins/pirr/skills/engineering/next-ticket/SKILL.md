...
6. **Optional flip and branch creation.** If the user accepts the recommendation:
   ...
   - Flip the ticket's status from `Open` to `In progress` (read the ticket file → edit the frontmatter `status:` to the **literal encoding** — the value set is `open | in-progress | done` per STORE.md and TICKET-FORMAT.md, so the flip writes `in-progress`, never a prose rendering like `in progress`. The flip is a mid-lifecycle working-tree edit, never its own commit — it rides along with the ticket's next real commit; end-of-lifecycle `→ Done` flips have no next commit to ride and are instead committed in the closing skill's gated close-out commit, per STORE.md). **Store edit and git commands stay sequential — never batched in parallel** (see STORE.md).
   - Otherwise, leave status unchanged and let the user pick differently.

7. **Standing ticket-start step.** Dispatch a research sub-agent as part of ticket start — a standing step, no per-ticket confirm. The sub-agent's brief: map the code and docs relevant to this ticket's Goal and Acceptance criteria, verify any external-toolchain assumptions the ticket or spec makes (stdlib APIs, build semantics, library behavior) against the installed toolchain, and **recommend the implementation mode — `/tdd` or `/implement` — naming what favors the recommended mode and what the alternative would cost, never a bare pick**. The recommendation introduces no new gate and no config knob: the user ratifies it at the plan-approval gate the chosen mode already owns, and declining routes to the other mode — expressed by rejecting the plan and asking for it — not through a separate confirm. Relay its findings before implementation starts.
